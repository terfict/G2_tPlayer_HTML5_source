// JavaScript Document - G204B2│g204B2_Video540x360Outline.js
////////////////////////////////////////////////////////
/******************************************************/
var NOU = NOU || {};
//-----------------------------------------------------
...
//------------------------
var g204B2 = NOU.namespace('NOU.modules.g204B2');



g204B2.flagStr = "hideOutline";
...
//操控大綱文字區塊$('.g204B2_outlineWrapper')
method.handlingOutline = function(){
	
	//outlineWrapperIniX改為不用↓↓↓
	/*改寫原因：因iPad以旭聯APP進入，G204B2版型讀取.g204B2_outlineWrapper的left屬性值，遭APP干擾，變成NaN*/
	//var outlineWrapperIniX = parseInt($('.g204B2_outlineWrapper').css('left'));
	...
	$('.showHideOutlineBtnDiv').on('mousedown',function(){
		...
		//
		//outlineWrapperIniX = parseInt($('.g204B2_outlineWrapper').css('left'));
		//alert(outlineWrapperIniX + '/' + g204B2.outlineOffset + '/' + (outlineWrapperIniX-g204B2.outlineOffset));
		
		
		
		//「大綱文字」 從 關閉的狀態，轉換到 開啟的狀態。
		if( g204B2.flagStr === "hideOutline" ){
			//
			$('.showHideOutlineBtnDiv').css({'background':'url(' + param.mainPath + 'g204B2_Video540x360Outline/images/hideOutline.png)'});
			//
			$('.showHideOutlineBtnDiv').animate({'left':0},280);
			//
			$('.g204B2_outlineWrapper').css({'display':'block','opacity':0.0}).animate({'opacity':1.0,'left':(g204B2.outlineWrapperIniX-g204B2.outlineOffset)},280, function(){
				//alert(outlineWrapperIniX + '/' + g204B2.outlineOffset + '/' + (outlineWrapperIniX-g204B2.outlineOffset));
			});
			//
		...
		}else if( g204B2.flagStr === "showOutline" ){
			//
			$('.showHideOutlineBtnDiv').css({'background':'url(' + param.mainPath + 'g204B2_Video540x360Outline/images/showOutline.png)'});
			//
			$('.showHideOutlineBtnDiv').animate({'left':showOutlineBtnIniX},280);
			//
			$('.g204B2_outlineWrapper').animate({'left':g204B2.outlineWrapperIniX,'opacity':0.0},280,function(){
				$('.g204B2_outlineWrapper').css({'display':'none','opacity':1.0});
			});
			//
...
$(document).ready(function(){
	...
	//1050826 - ●○$(document).ready()之後，$('.g204B2_outlineWrapper')，小於斷點就出現，大於斷點就隱藏 。
	if( $(window).width() <= param.breakpoint ){
		$('.g204B2_outlineWrapper').css({'display':'block'});
	}else{
		$('.g204B2_outlineWrapper').css({'display':'none'});
	}
	...
	//-----------------------------------------
	//$('.g204B2_outline').fadeTo(0,0.0).delay(1000).fadeTo(360,1.0);
	//-----------------------------------------
...
$(window).on('resize', function(){
	...
	if( $(window).width() <= param.breakpoint ){
		...
		//1050826 - ●○$('.g204B2_outlineWrapper')，小於斷點就出現，大於斷點，由純CSS處理。
		$('.g204B2_outlineWrapper').css({'display':'block'});
...
method.locateMagnifierIcon = function(){ //alert('G204B2');
	...
	$('.toBigPic').each(function(index, element) {
		...
		//1050826 - G204B2不用調用method.triggerToBig()
