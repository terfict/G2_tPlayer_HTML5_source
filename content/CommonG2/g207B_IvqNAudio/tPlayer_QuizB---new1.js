...
function nextQuiz(){
	...
	//====================================================================●
	//●1060906隱藏 - 若加此段會有Error - 好像error不是因此造成。而是某題無語音所造成。
	//====================================================================●
	$.each(param.ivqHasMediaArr, function(index){
		tPlayer.pause( param.ivqHasMediaArr[index][2]+1 );
		tPlayer.seek( 0, param.ivqHasMediaArr[index][2]+1 );
...
//========↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
//●○●○●○1060725-修改●○●○●○ - 用來播放 影音(InVideoQuiz)。覆蓋tPlayer.js同名方法。
tPlayer.play = function(index){
	...
	//●○●○●○1060725 -InVideoQuiz影片繼續播放時，題目的語音檔需seek到語音檔起點(0)，停止播放，並隱藏播放介面。
	//========================================《《《《《《《《《《《《《《《《《《《《《《《《《《《《《《《
...
tPlayer.play2 = function(index){
	...
	//$('#quizDiv').css({'display':'none'}).empty().remove();  //●○●○●○<===========1060725 - 不可清掉評量提
	//$('#quizDiv, quizContainer').css({'display':'none'});
...
method.mediaCompleted = function(index){
	if(index !== 0){
		...
		//●需減1，否則播完後，滑鼠按下play鈕，會播下一段audio
		//==========================================●
		playBtnState(index-1);
		//==========================================●
...
function initBtnState(index){
	...
	//alert(utils.isAndroid + '/' + g207B.audioAutoPlay);
	
	
	if( (utils.isMobile && utils.isAndroid) || (utils.isMobile && utils.isIOS) ){
		 g207B.audioAutoPlay = false; 
	}
	//alert(g207B.audioAutoPlay);
	
	
	
	//●○ 語音是否自動播放 - 自動播放：true│不自動播放：false
	if( g207B.audioAutoPlay ){
		
		//alert( param.ivqHasMediaArr[index][2]+1 );
		//●1060905
		//==============================================================●
		setTimeout(function(){
			//
			...
			//alert( param.ivqHasMediaArr[index][2]+1 +'/' + index );
			
			//●●●1060906 - 引數改為index
			//tPlayer.play2( index );
			
			//
			g207B.audioStatus = 'play';  
			//
			playBtnOffEvent();
			//
			pauseBtnState(index);
...
function playBtnState(index){
	...
	$('.playBtn img').css({'opacity':1.0})
		...
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_playBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_playBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_playBtn.png').css({'opacity':0.5});
			
			
			tPlayer.play2( param.ivqHasMediaArr[index][2]+1 );  //alert(param.ivqHasMediaArr[index][2]+1);
			g207B.audioStatus = 'play';  
...
function pauseBtnState(index){
	...
	$('.pauseBtn img').css({'opacity':1.0})
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_pauseBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_pauseBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_pauseBtn.png').css({'opacity':0.5});

								
			tPlayer.pause( param.ivqHasMediaArr[index][2]+1 );
			g207B.audioStatus = 'pause';
...
function stopBtnState(index){
	...
	$('.stopBtn img').css({'opacity':1.0})
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_stopBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_stopBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_stopBtn.png').css({'opacity':0.5});
			...
			if( param.ivqHasMediaArr[index][1] === _currentQuiz ){
				...
				var htmlStr = 
					...
					'<div class="btnWrapper">'+
						'<span class="playBtn"><img src="' + param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_playBtn.png" alt="play按鈕"></span>'+
						'<span class="pauseBtn"><img src="' + param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_pauseBtn.png" alt="pause按鈕"></span>'+
						'<span class="stopBtn"><img src="' + param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_stopBtn.png" alt="stop按鈕"></span>'+
					'</div>'+
					'</div>';
				...
				//alert(param.mainPath + 'g207B_IvqNAudio/images/simplePlayer_stopBtn.png');
