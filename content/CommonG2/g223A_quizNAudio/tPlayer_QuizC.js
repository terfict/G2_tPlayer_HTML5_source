...
function initBtnState(index){
	...
	//alert(utils.isAndroid + '/' + g223A.audioAutoPlay);
	
	
	if( (utils.isMobile && utils.isAndroid) || (utils.isMobile && utils.isIOS) ){
		 g223A.audioAutoPlay = false; 
	}else{
		g223A.audioAutoPlay = true; 
	}
	//alert('g223A.audioAutoPlay : ' + g223A.audioAutoPlay);
	
	
	
	//●○ 語音是否自動播放 - 自動播放：true│不自動播放：false
	if( g223A.audioAutoPlay ){ 
...
function playBtnState(index){
	...
	$('.playBtn img').css({'opacity':1.0})
		...
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_playBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_playBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_playBtn.png').css({'opacity':0.5});
			//alert(param.ivqHasMediaArr[index][2]);
			
			//
			if( g223A.audioStatus === 'stop' ){
				tPlayer.seek( 0, param.ivqHasMediaArr[index][2] );
			}
...
function pauseBtnState(index){
	...
	$('.pauseBtn img').css({'opacity':1.0})
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_pauseBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_pauseBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_pauseBtn.png').css({'opacity':0.5});

								
			tPlayer.pause( param.ivqHasMediaArr[index][2] );
			g223A.audioStatus = 'pause';
...
function stopBtnState(index){
	...
	$('.stopBtn img').css({'opacity':1.0})
		.on('mouseover',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_stopBtn_over.png');
		})
		.on('mouseout',function(){
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_stopBtn.png');
		})
		
		.on('mousedown', function(){ 
			$(this).attr('src',param.mainPath + 'g223A_quizNAudio/images/simplePlayer_stopBtn.png').css({'opacity':0.5});
			...
			if( param.ivqHasMediaArr[index][1] === _currentQuiz ){
				...
				var htmlStr = 
					...
					'<div class="btnWrapper">'+
						'<span class="playBtn"><img src="' + param.mainPath + 'g223A_quizNAudio/images/simplePlayer_playBtn.png" alt="play按鈕"></span>'+
						'<span class="pauseBtn"><img src="' + param.mainPath + 'g223A_quizNAudio/images/simplePlayer_pauseBtn.png" alt="pause按鈕"></span>'+
						'<span class="stopBtn"><img src="' + param.mainPath + 'g223A_quizNAudio/images/simplePlayer_stopBtn.png" alt="stop按鈕"></span>'+
					'</div>'+
					'</div>';
				...
				//alert(param.mainPath + 'g223A_quizNAudio/images/simplePlayer_stopBtn.png');
