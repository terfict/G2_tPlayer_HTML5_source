...
g218A.createCardTitleArrNElse = function(){  
	...
	$('.g218A_card').each(function(index) { 
		...
		//●●●將小標題.g218A_cardTitle存到g218A.cardTitleArr
		g218A.cardTitleArr[index] = $('.g218A_card:eq(' + index + ') .g218A_cardTitle').html(); 
		
		
		//●●●內文.g218A_text存到g218A.cardTextArr
...
g218A.scrollControl = function(){
	...
	//●○1060104 - 計算$('.g218A_menuZone')捲軸捲動距離，以及捲動
			//===========================================================
			...
			$('.g218A_card').each(function(index){ 
				...
				if( index < g218A.currCardIndex ){
					scrollTopOffset += ( $('.g218A_menuItem:eq(' + index + ')').height() + parseInt($('.g218A_menuItem:eq(' + index + ')').css('padding-top')) + parseInt($('.g218A_menuItem:eq(' + index + ')').css('padding-bottom')) + parseInt($('.g218A_menuItem:eq(' + index + ')').css('border-bottom-width')) );
				}
...
//1051026
//滑鼠按下數字按鈕的事件處理 《《《 按下.g218A_menuItem也調用此事件處理
...
//1051018
//數字按鈕.g218A_numButton、.g218A_menuButton開啟選單列表按鈕 的滑鼠事件處理
	...
	//●○◎※●○◎※●○◎※ mousedown ●○◎※●○◎※●○◎※
		...
		$.each(g218A.timeCodeArr, function(itemIndex) { 
			...
			var menuZoneHtml = '<div class="g218A_menuItem">'+
								'<span class="numOrder"><span class="g218A_numButton"></span></span>'+
			...
			//數字按鈕 附加 序號
			$('.g218A_numButton:eq(' + itemIndex + ')').html(itemIndex+1);
			//選單項目 附加 小標題
			$('.g218A_menuItem:eq('+ itemIndex + ') .titleStr').addClass('fontSize').append( g218A.cardTitleArr[itemIndex] );
			...
			if( $('.g218A_card:eq('+ itemIndex + ') .g218A_text').html() !== '' && $('.g218A_card:eq('+ itemIndex + ') .g218A_text').get(0) !== undefined ){
				$('.g218A_menuItem:eq('+ itemIndex + ') .text').addClass('fontSize').append( g218A.cardTextArr[itemIndex] );
				//↑↑↑1060124 - 》》》 ●○在此附加.fontSize類別，以觸發字級縮放功能。
			...
			//●1060120 - 設置.g218A_top捲動至頂端按鈕
			//=============================================================
			//累加所有.g218A_menuItem的高度
			if( itemIndex < g218A.totalCard ){
				//alert( $('.g218A_menuItem:eq('+ itemIndex + ')').height() );
				menuItemHeightSum += $('.g218A_menuItem:eq('+ itemIndex + ')').height();
				
			}
			
			//累加完成時，由$('.g218A_menuZone')將 '<div class="top"></div>' 予以 append
			if( itemIndex === g218A.totalCard-1 ){
				//alert( $('.g218A_menuZone').height() +'/' + menuItemHeightSum );
				
				//
				//if( $('.g218A_menuZone').height() < menuItemHeightSum ){ 
				
					//●○附加「捲動到頂端」.g218A_top按鈕
					$('.g218A_menuZone').append('<div class="g218A_top"></div>');
				...
				//-----------------------------------------------------------------------
				$('.g218A_menuZone').after('<span class="fontSizeSwitcher"></span>');
			...
			//
			$('.g218A_top').on('mousedown',function(){
				$('.g218A_menuZone').animate({'scrollTop':0},380);
			});
			//=============================================================
...
g218A.originalWinOnResize = function(){
	...
	$(window).on('resize',function(){
		...
		//
			...
			g218A.mediaHeight = g218A.mediaWidth/g218A.mediaW2hRatio;
			g218A.menuZoneCurrWidth = g218A.pageInitWidth - g218A.mediaWidth ;
...
g218A.controlMouseDown = function(index){ 
	...
	//●○●○●○1060123
	//整頁全畫面時： ●改為使用 g218A.mediaWidth 來判斷
	if( g218A.pageWidthState === 'fullPage' ){
		
		//●○1060123
		//==============================================================
		if( g218A.mediaWidth < 320){
			$('.playbackDiv').css({'display':'none'});	
	...
	//一般畫面時： ●改為使用 g218A.mediaWidth 來判斷
	}else if( g218A.pageWidthState === 'original' ){
		
		//●○1060123
		//==============================================================
		if( g218A.mediaWidth < 280){
			$('.playbackDiv').css({'display':'none'});	
	...
	//
	$('.g218A_menuZone, .g218A_controlBar').animate({'width':g218A.menuZoneCurrWidth - g218A.widthOffset},360,function(){
...
g218A.pageWidthControlEvent = function(_toPageFlag){
	...
	jQuery( ":animated" ).clearQueue();
  	jQuery( ":animated" ).stop();
		...
		//.g218A_menuZone 目前寬度 - ●○讓g218A.menuZoneCurrWidth
		g218A.menuZoneCurrWidth = $('.g218A_menuZone').width();
		...
		if(_toPageFlag === 'toFullPage'){
			...
			$('.g218A_menuZone, .g218A_controlBar').animate({'width':g218A.menuZoneCurrWidth},360);
			
			//●○1060105
			//$('.g218A_menuZone').height( $(window).height() - $('.g218A_header').height() - $('.g218A_controlBar').height() );
			
			$('.g218A_menuZone').animate( {
				'height':$(window).height() - $('.g218A_header').height() - $('.g218A_controlBar').height()},360, function(){
					//
					g218A.scrollControl();
			...
			$('.g218A_menuZone, .g218A_controlBar').animate({'width':g218A.menuZoneCurrWidth},360);
			//alert( parseInt($('.g218A_controlBar').css('height')) );
			
			$('.g218A_menuZone').animate( {
				'height': parseInt($('.g218A_controlBar').css('height'))},360, function(){
		...
		//※※※1060106
		var controlBarW = (g218A.menuZoneCurrWidth)/pageToMediaWidthRatio;
...
g218A.fullPageWinOnResize = function(){ 
	...
	$(window).on('resize',function(){
		...
		if( $(window).width() > param.breakpoint && g218A.pageWidthState === 'fullPage' ){
			...
			g218A.mediaHeight = g218A.mediaWidth/g218A.mediaW2hRatio;
			g218A.menuZoneCurrWidth = g218A.pageCurrWidth - g218A.mediaWidth ;
...
g218A.plusMinusMouseEvent = function(){
	...
	$('.g218A_plus').on('mouseover',function(){
		$('.g218A_plus img').attr('src',param.mainPath + 'g218A_PresentationVideoCue/images/plusBtnImg_over.png');
	});
	
	//
	$('.g218A_plus').on('mouseout',function(){
		$('.g218A_plus img').attr('src',param.mainPath + 'g218A_PresentationVideoCue/images/plusBtnImg.png');
	});
	...
	$('.g218A_minus').on('mouseover',function(){
		$('.g218A_minus img').attr('src',param.mainPath + 'g218A_PresentationVideoCue/images/minusBtnImg_over.png');
	});
	
	//
	$('.g218A_minus').on('mouseout',function(){
		$('.g218A_minus img').attr('src',param.mainPath + 'g218A_PresentationVideoCue/images/minusBtnImg.png');
	});
...
g218A.helpMouseEvent = function(){
	$('.g218A_help img').on('mouseover',function(){
		$(this).attr('src', param.mainPath + 'g218A_PresentationVideoCue/images/help_over.png');
	});
	$('.g218A_help img').on('mouseout',function(){
		$(this).attr('src', param.mainPath + 'g218A_PresentationVideoCue/images/help.png');
	});
...
g218A.createImg = function(){
	//
	$('.g218A_nouIcon').html('<img src="' + param.mainPath + 'g218A_PresentationVideoCue/images/nou_media.png" alt="NOU Media Icon.">');
	//
	$('.g218A_minus').html('<img src="' + param.mainPath + 'g218A_PresentationVideoCue/images/minusBtnImg.png" alt="縮小頁面">');
	//
	$('.g218A_plus').html('<img src="' + param.mainPath + 'g218A_PresentationVideoCue/images/plusBtnImg.png" alt="加大頁面">');
	//
	$('.g218A_help').html('<a><img src="' + param.mainPath + 'g218A_PresentationVideoCue/images/help.png" alt="加大頁面"></a>');
