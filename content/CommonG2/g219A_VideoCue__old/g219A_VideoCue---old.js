...
g219A.createCardTitleArrNElse = function(){  
	...
	//定義.g219A_menuButton的HTML字串，附加到.g219A_numButtonGroup裡面
	var menuButtonHtml = '<div class="g219A_menuButton" alt="切換為「條列選單」" title="切換為「條列選單」"></div>';
	$('.g219A_numButtonGroup').append( menuButtonHtml );
	
	//定義.g219A_returnNumButton的HTML字串，附加到.g219A_numButtonGroup裡面
	var returnNumButtonHtml = '<div class="g219A_returnNumButton" alt="切換為「按鈕選單」" title="切換為「按鈕選單」"></div>';
	$('.g219A_numButtonGroup').append( returnNumButtonHtml );
	
	
	
	//●全部的.g219A_card，都append到.g219A_cardGroupShow裡
	$('.g219A_card').each(function(index) { 
		...
		//●全部的.g219A_card，都append到.g219A_cardGroupShow裡
		//$('.g219A_cardGroupShow').append( $('.g219A_card:eq(' + (index) + ')') );
		
		
		//●○●○●○1051130 - g219A_cardGroupShow於本版型用不著
		//---------------------------------------------------
		//$('.g219A_cardGroupShow').css({'display':'none'});
		//---------------------------------------------------
		...
		//●●●1051014 - 附加數字按鈕
		//==============================================================
		var numButtonHtml = '<span class="g219A_numButton"></span>';
		$('.g219A_numButtonGroup').append( numButtonHtml );
		//
		$('.g219A_numButton:eq(' + index + ')').html(index+1);
		//==============================================================
	...
	//●○1060104 - 計算$('.g219A_menuZone')捲軸捲動距離
			...
			$('.g219A_card').each(function(index){ 
				...
				if( index < g219A.currCardIndex ){
					scrollTopOffset += $('.g219A_menuItem:eq(' + index + ')').height();
				}
...
g219A.numButtonOverOut = function(){
	...
	$.each(g219A.timeCodeArr,function(timeCodeIndex){
		...
		$('.g219A_numButton:eq(' + timeCodeIndex + ')').on('mouseover',function(){
			...
			//1051110 - 滑鼠移到數字按鈕上面，出現標題及標題底色 (G213A才有，G219A沒有)
			//.g219A_showCueTitle
			if( !utils.isMobile ){
				$(document.body).append('<div class="g219A_showCueTitle"></div>');
				$('.g219A_showCueTitle').html( g219A.cardTitleArr[timeCodeIndex] );
				
				var posLeft = $('.g219A_numButtonGroup').offset().left;
				var posTop = $('.g219A_numButtonGroup').offset().top - $('.g219A_showCueTitle').height() - parseInt($('.g219A_showCueTitle').css('padding-top')) - parseInt($('.g219A_showCueTitle').css('padding-bottom'));
				$('.g219A_showCueTitle')
		...
		$('.g219A_numButton:eq(' + timeCodeIndex + ')').on('mouseout',function(){
			...
			$('.g219A_showCueTitle').empty().remove();
...
g219A.buttonsMouseEvent = function(){
	...
	$.each(g219A.timeCodeArr,function(timeCodeIndex){
		...
		$('.g219A_numButton:eq(' + timeCodeIndex + ')').on('mousedown',function(){
			...
			//1051111 - @Android Mobile - 點擊數字按鈕後，隨即關閉$('.g219A_showCueTitle')，避免小標題文字持續停留在畫面上
			//--------------------------------------------------------------
			if( !utils.isMobile ){
				if( $('.g219A_showCueTitle').get(0) !== undefined ){
					$('.g219A_showCueTitle').empty().remove();
				}
			}
			//--------------------------------------------------------------
	...
	//●.g219A_menuButton按鈕的滑鼠事件處理
	//==========================================================================
	
	//mouseover
	$('.g219A_menuButton').on('mouseover',function(){
		$('.g219A_menuButton').addClass('g219A_menuButton_over');
	});
	
	//mouseout
	$('.g219A_menuButton').on('mouseout',function(){
		$('.g219A_menuButton').removeClass('g219A_menuButton_over');
	});
	
	
	
	//●○◎※●○◎※●○◎※ mousedown ●○◎※●○◎※●○◎※
	$('.g219A_menuButton').on('mousedown',function(){
		
		//1051111
		g219A.menuHasExpanded = true;
		
		//1051111
		g219A.numButtonOverOutOff();
		
		
		$('.g219A_menuButton').removeClass('g219A_menuButton_over');
		//
		$('.g219A_menuButton').css({'display':'none'});
		$('.g219A_returnNumButton').css({'display':'inline-block'});

		
		//1051111
		$('.g219A_numButtonGroup').insertBefore($('.g219A_cardGroupShow'));
		$('.g219A_menuZone').insertAfter($('.g219A_numButtonGroup'));
		//$('.g219A_menuZone').fadeIn(300).css({'display':'table'});
		...
		$.each(g219A.timeCodeArr, function(itemIndex) { 
			...
			//
			$('.g219A_menuItem:eq(' + itemIndex + ') .numOrder').append( $('.g219A_numButton:eq(' + (0) + ')') );
			//
			$('.g219A_menuItem:eq('+ itemIndex + ') .titleStr').append( g219A.cardTitleArr[itemIndex] );
			...
			if( $('.g219A_card:eq('+ itemIndex + ') .g219A_text').html() !== '' && $('.g219A_card:eq('+ itemIndex + ') .g219A_text').get(0) !== undefined ){
				$('.g219A_menuItem:eq('+ itemIndex + ') .text').append( g219A.cardTextArr[itemIndex] );
		...
		//
		$('.g219A_numButtonGroup').addClass('g219A_numButtonGroup_list');
		//
		$('.g219A_numButton').addClass('g219A_numButton_list');
	...
	//●.g219A_returnNumButton按鈕的滑鼠事件處理
	//==========================================================================
	//mouseover
	$('.g219A_returnNumButton').on('mouseover',function(){
		$('.g219A_returnNumButton').addClass('g219A_returnNumButton_over');
	});
	
	//mouseout
	$('.g219A_returnNumButton').on('mouseout',function(){
		$('.g219A_returnNumButton').removeClass('g219A_returnNumButton_over');
	});
	
	//mousedown
	$('.g219A_returnNumButton').on('mousedown',function(){
		...
		$('.g219A_returnNumButton').removeClass('g219A_returnNumButton_over');
		//
		$('.g219A_returnNumButton').css({'display':'none'});
		$('.g219A_menuButton').css({'display':'inline-block'});
		
		//
		//==============================
		g219A.offMenuItemMEvent();
		//==============================
		
		//
		$('.g219A_menuZone').fadeOut(300, function(){
			$('.g219A_menuZone').empty();
		});
		...
		$.each(g219A.timeCodeArr, function(itemIndex) { 
			//
			$('.g219A_numButtonGroup').append( $('.g219A_numButton:eq(' + (itemIndex) + ')') );
		});
		
		
		//
		$('.g219A_numButtonGroup').removeClass('g219A_numButtonGroup_list');
		//
		$('.g219A_numButton').removeClass('g219A_numButton_list');
		
		
		//1051111
		$('.g219A_cardGroupShow').insertBefore($('.g219A_numButtonGroup'));
		//
		//1051111
		/*if( !param.playingFlagArr[g219A.mediaIndex] ){
			tPlayer.play(g219A.mediaIndex);
...
//●○●○●○ 操控媒體區塊的寬高 ●○●○● - 按下.g219A_plus、.g219A_minus 按鈕接調用此方法。
...
g219A.originalWinOnResize = function(){
	...
	$(window).on('resize',function(){
		...
		//
			...
			g219A.mediaHeight = g219A.mediaWidth/g219A.mediaW2hRatio;
			g219A.menuZoneCurrWidth = g219A.pageInitWidth - g219A.mediaWidth - 1;
...
g219A.controlMouseDown = function(index){ 
	...
	//
	$('.g219A_menuZone, .g219A_controlBar').animate({'width':g219A.menuZoneCurrWidth-3},360,function(){
...
g219A.pageWidthControlEvent = function(_toPageFlag){
	...
	jQuery( ":animated" ).clearQueue();
  	jQuery( ":animated" ).stop();
		...
		//.g219A_menuZone 目前寬度 - ●○讓g219A.menuZoneCurrWidth-3
		g219A.menuZoneCurrWidth = $('.g219A_menuZone').width();
		...
		if(_toPageFlag === 'toFullPage'){
			...
			$('.g219A_menuZone, .g219A_controlBar').animate({'width':g219A.menuZoneCurrWidth-3},360);
			
			//●○1060105
			//$('.g219A_menuZone').height( $(window).height() - $('.g219A_header').height() - $('.g219A_controlBar').height() );
			
			$('.g219A_menuZone').animate( {
				'height':$(window).height() - $('.g219A_header').height() - $('.g219A_controlBar').height(),
				'width':g219A.menuZoneCurrWidth-3},360, function(){
			...
			$('.g219A_menuZone').animate( {
				'height': parseInt($('.g219A_controlBar').css('min-height')),
				'width':g219A.menuZoneCurrWidth-3},360, function(){
		...
		//※※※1060106
		var controlBarW = (g219A.menuZoneCurrWidth-3)/pageToMediaWidthRatio;
...
g219A.fullPageWinOnResize = function(){ 
	...
	$(window).on('resize',function(){
		...
		if( $(window).width() > param.breakpoint && g219A.pageWidthState === 'fullPage' ){
			...
			g219A.mediaHeight = g219A.mediaWidth/g219A.mediaW2hRatio;
			g219A.menuZoneCurrWidth = g219A.pageCurrWidth - g219A.mediaWidth - 1;
...
$(document).ready(function(){
	...
	$('.mediaWrapper').each(function(mediaIndex) {
		...
		if(mediaIndex === 0){
			...
			//解決：mediaWrapper過高所產生的mediaWrapper和g219A_cardGroupShow之間可能產生的10px間隙。
			//G219A版型從G213A轉製而來，G213A有上述問題。G219A也許沒有？？？
			//=====================================================================
			/*var tmpHeight = $('.mediaWrapper:eq(' + g219A.mediaIndex + ') .mediaHeader').height() + $('.mediaWrapper:eq(' + g219A.mediaIndex + ') .playbackDiv').height() + $('.mediaWrapper:eq(' + g219A.mediaIndex + ') .mediaDiv').height();
			$('.mediaWrapper:eq(' + g219A.mediaIndex + ')').css({'height':tmpHeight});*/
			//=====================================================================
			...
			//1051208 - G219A版型由G213A轉製而成，但G219A沒有純粹按鈕選單的狀態，
			//所以直接觸發$('.g219A_menuButton')的mousedown事件，進入列表選單的狀態。
			//並隱藏$('.g219A_numButtonGroup')。 .g219A_numButtonGrou為所有.numButton的容器。
			//========================================================
			//========================================================
			$('.g219A_menuButton').trigger('mousedown');
			$('.g219A_numButtonGroup').css({'display':'none'});
			//========================================================
			//========================================================
